...
class ThreadCliente extends Thread {
    ...
    /* Código que a thread vai executar após ser iniciada */
    @Override
    public void run() {
        int arquivo, op;
        ...
        for (int i = 0; i < 10; i++) { // gerar 10 requisições
            ...
            System.out.printf("Cliente %d quer fazer uma %s no arquivo %s.%n", cliente.id, op == LEITURA ? "leitura" : "escrita", nomeArquivo(arquivo));
            
            
            try { // chamar método de requisição do objeto remoto
